...
function InvitationCreatePage() {
  ...
  const formik = useFormik<InvitationInterface>({
    initialValues: {
      status: '',
      invited_at: new Date(new Date().toDateString()),
      accepted_at: new Date(new Date().toDateString()),
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          ...
          <FormControl id="invited_at" mb="4">
            <FormLabel fontSize="1rem" fontWeight={600}>
              Invited At
            </FormLabel>
            <DatePicker
              selected={formik.values?.invited_at ? new Date(formik.values?.invited_at) : null}
              onChange={(value: Date) => formik.setFieldValue('invited_at', value)}
            />
          </FormControl>
          <FormControl id="accepted_at" mb="4">
            <FormLabel fontSize="1rem" fontWeight={600}>
              Accepted At
            </FormLabel>
            <DatePicker
              selected={formik.values?.accepted_at ? new Date(formik.values?.accepted_at) : null}
              onChange={(value: Date) => formik.setFieldValue('accepted_at', value)}
            />
          </FormControl>
